...
import { useListingItemMutation } from '../../redux/reducers/listingReducer';
...
const CreateListing = () => {
  ...
  const [createListing, result] = useListingItemMutation();
  ...
  useEffect(() => {
    if (result.isError) {
      setimageUploadError(result.error.data.message);
    }
    if (result.isSuccess) {
      navigate(`/listing/${result.data.listing._id}`);
    }
  }, [result]);
